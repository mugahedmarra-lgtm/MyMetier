        ...
        <div class="bg-indigo-600 rounded-2xl shadow-md border border-indigo-700 p-8 mb-8 text-white relative overflow-hidden">
            ...
            <div class="relative z-10 flex flex-col md:flex-row items-center gap-6">
                <div class="w-20 h-20 bg-indigo-500 rounded-full flex items-center justify-center border-4 border-indigo-400 shadow-inner">
                    <span class="text-3xl font-bold">{{ mb_substr($user?->name ?? 'G', 0, 1) }}</span>
                </div>
                <div class="text-center md:text-right">
                    <h1 class="text-3xl font-extrabold tracking-tight mb-1">أهلاً بك، {{ $user?->name ?? 'Guest' }}</h1>
        ...
        <div class="flex flex-col lg:flex-row gap-8">
            ...
            <aside class="w-full lg:w-1/4 shrink-0">
                <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-3 lg:sticky lg:top-24 flex lg:flex-col overflow-x-auto lg:overflow-visible no-scrollbar">
                    <button wire:click="setTab('favorites')" class="flex items-center gap-3 px-5 py-4 rounded-xl text-right font-medium transition whitespace-nowrap lg:whitespace-normal {{ $activeTab === 'favorites' ? 'bg-indigo-50 text-indigo-700 shadow-sm border border-indigo-100' : 'text-gray-600 hover:bg-gray-50' }}">
                        <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 {{ $activeTab === 'favorites' ? 'text-indigo-600' : 'text-gray-400' }}" fill="{{ $activeTab === 'favorites' ? 'currentColor' : 'none' }}" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z" />
                        </svg>
                        المفضلة
                        <span class="mr-auto bg-gray-100 text-gray-600 text-xs px-2 py-0.5 rounded-full">{{ $favorites->count() }}</span>
                    </button>
                    
                    <button wire:click="setTab('requests')" class="flex items-center gap-3 px-5 py-4 rounded-xl text-right font-medium transition whitespace-nowrap lg:whitespace-normal {{ $activeTab === 'requests' ? 'bg-indigo-50 text-indigo-700 shadow-sm border border-indigo-100' : 'text-gray-600 hover:bg-gray-50' }}">
                        <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 {{ $activeTab === 'requests' ? 'text-indigo-600' : 'text-gray-400' }}" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                        </svg>
                        طلباتي
                        <span class="mr-auto bg-gray-100 text-gray-600 text-xs px-2 py-0.5 rounded-full">{{ $requests->count() }}</span>
                    </button>
                    
                    <button wire:click="setTab('reviews')" class="flex items-center gap-3 px-5 py-4 rounded-xl text-right font-medium transition whitespace-nowrap lg:whitespace-normal {{ $activeTab === 'reviews' ? 'bg-indigo-50 text-indigo-700 shadow-sm border border-indigo-100' : 'text-gray-600 hover:bg-gray-50' }}">
                        <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 {{ $activeTab === 'reviews' ? 'text-indigo-600' : 'text-gray-400' }}" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M8.625 12a.375.375 0 11-.75 0 .375.375 0 01.75 0zm0 0H8.25m4.125 0a.375.375 0 11-.75 0 .375.375 0 01.75 0zm0 0H12m4.125 0a.375.375 0 11-.75 0 .375.375 0 01.75 0zm0 0h-.375M21 12c0 4.556-4.03 8.25-9 8.25a9.764 9.764 0 01-2.555-.337A5.972 5.972 0 015.41 20.97a5.969 5.969 0 01-.474-.065 4.48 4.48 0 00.978-2.025c.09-.457-.133-.901-.467-1.226C3.93 16.178 3 14.189 3 12c0-4.556 4.03-8.25 9-8.25s9 3.694 9 8.25z" />
                        </svg>
                        تقييماتي
                        <span class="mr-auto bg-gray-100 text-gray-600 text-xs px-2 py-0.5 rounded-full">{{ $reviews->count() }}</span>
                    </button>
                </div>
                ...
                @elseif(!$user->isProfessional() && !$user->isContractor())
                    @php
                    ...
                    @elseif($profileStatus === 'rejected')
                        ...
                        <div class="mt-6 bg-amber-50 rounded-2xl p-5 border border-amber-200 text-center">
                            ...
                            <a href="{{ route('upgrade') }}" class="inline-block text-sm font-bold text-amber-700 bg-white border border-amber-200 hover:bg-amber-50 px-4 py-2 rounded-lg transition shadow-sm w-full">تعديل وإعادة الإرسال</a>
                        </div>
                    @else
                        {{-- Fresh Customer — Show Upgrade CTAs --}}
                        <div class="mt-6 bg-gradient-to-br from-indigo-50 to-white rounded-2xl p-5 border border-indigo-100 shadow-sm relative overflow-hidden">
                            ...
                            <div class="flex flex-col gap-2.5 relative z-10">
                                <a href="{{ route('upgrade', ['type' => 'professional']) }}" class="w-full flex justify-center items-center bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-2.5 px-4 rounded-xl transition shadow-sm text-sm">التسجيل كمهني</a>
                                <a href="{{ route('upgrade', ['type' => 'contractor']) }}" class="w-full flex justify-center items-center bg-white hover:bg-gray-50 text-indigo-700 border border-indigo-200 font-bold py-2.5 px-4 rounded-xl transition shadow-sm text-sm">التسجيل كمقاول</a>
                            </div>
                ...
                @elseif($user->isProfessional())
                    <div class="mt-6 bg-emerald-50 rounded-2xl p-5 border border-emerald-100 text-center">
                        ...
                        <a href="{{ route('pro.dashboard') }}" class="inline-block text-sm font-bold text-emerald-700 bg-white border border-emerald-200 hover:bg-emerald-50 px-4 py-2 rounded-lg transition shadow-sm w-full">الانتقال للوحة المهنيين</a>
                    </div>
                @elseif($user->isContractor())
                    <div class="mt-6 bg-blue-50 rounded-2xl p-5 border border-blue-100 text-center">
                        ...
                        <a href="{{ route('pro.dashboard') }}" class="inline-block text-sm font-bold text-blue-700 bg-white border border-blue-200 hover:bg-blue-50 px-4 py-2 rounded-lg transition shadow-sm w-full">الانتقال للوحة المقاولين</a>
                    </div>
                @endif
            </aside>
                        ...
                        @else
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                                @foreach($favorites as $fav)
                                    @php $profile = $fav->professionalProfile; @endphp
                                    <div class="bg-white rounded-2xl shadow-md border border-gray-100 hover:shadow-xl hover:border-indigo-100 hover:-translate-y-1 transition-all duration-300 overflow-hidden flex flex-col h-full relative">
                                        ...
                                        <div class="p-6 flex flex-col flex-1">
                                            {{-- Header: Name + Verification + Remove --}}
                                            <div class="flex items-start justify-between mb-4">
                                                <div class="min-w-0 flex-1">
                                                    <h3 class="text-lg font-bold text-gray-900 truncate">{{ $profile->display_name }}</h3>
                                                    <p class="text-sm text-gray-500 mt-1">{{ $profile->category?->name ?? '—' }}</p>
                                                </div>
                                                <div class="flex items-center gap-2">
                                                    ...
                                                    <button wire:click.prevent="removeFavorite({{ $fav->id }})" class="p-1.5 rounded-full hover:bg-red-50 text-red-500 transition duration-300 focus:outline-none flex-shrink-0 relative group" title="إزالة من المفضلة" wire:loading.attr="disabled">
                                                        ...
                                                        <span wire:loading wire:target="removeFavorite({{ $fav->id }})" class="absolute inset-0 flex items-center justify-center bg-white/80 rounded-full">
                                    ...
                                    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 relative">
                                        ...
                                        @else
                                            ...
                                            <div class="flex justify-between items-start mb-4">
                                                <div>
                                                    <h3 class="font-bold text-gray-900 text-lg mb-1">{{ $req->title }}</h3>
                                                    <div class="flex gap-2 text-sm text-gray-500">
                                                        <span>{{ optional($req->category)->name }}</span>
                                                        <span>•</span>
                                                        <span>{{ optional($req->city)->name }}</span>
                                                    </div>
                                                </div>
                                                <div>
                                                    @if($req->status === 'open')
                                            ...
                                            <p class="text-gray-600 text-sm whitespace-pre-line mb-6 bg-gray-50 p-4 rounded-xl border border-gray-100">{{ $req->description }}</p>
                                            ...
                                            <div class="flex items-center justify-between text-sm">
                                                ...
                                                @if($req->status === 'open')
                                                    <div class="flex gap-2">
                                                        <button wire:click="editRequest({{ $req->id }})" class="text-indigo-600 hover:bg-indigo-50 font-medium px-4 py-2 rounded-lg transition border border-transparent hover:border-indigo-100">
                                                            تعديل
                                                        </button>
                                                        <button wire:click="closeRequest({{ $req->id }})" wire:confirm="هل أنت متأكد من إغلاق هذا الطلب نهائياً؟" class="text-red-600 hover:bg-red-50 font-medium px-4 py-2 rounded-lg transition border border-transparent hover:border-red-100">
                                                            إغلاق الطلب
                                                        </button>
                                                    </div>
                                                @endif
                                ...
                                @foreach($reviews as $rev)
                                    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 hover:shadow-md transition">
                                        <div class="flex justify-between items-start mb-3">
                                            <div class="flex items-center gap-3">
                                                <div class="w-10 h-10 rounded-full bg-indigo-50 text-indigo-600 flex items-center justify-center font-bold text-lg">
                                                    {{ mb_substr($rev->professionalProfile?->display_name ?? 'U', 0, 1) }}
                                                </div>
                                                <div>
                                                    <h3 class="font-bold text-gray-900">{{ $rev->professionalProfile?->display_name ?? 'غير معروف' }}</h3>
                                                    <a href="{{ route('profile.show', $rev->professional_profile_id) }}" class="text-xs text-indigo-500 hover:underline">عرض الملف</a>
                                                </div>
                                            </div>
                                        ...
                                        <div class="flex items-center gap-1 mb-3 text-amber-400">
                                            @for($i = 1; $i <= 5; $i++)
                                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="{{ $i <= $rev->rating ? 'currentColor' : 'none' }}" stroke="currentColor" stroke-width="1.5" class="w-4 h-4">
                                        ...
                                        @if($rev->comment)
                                            <p class="text-gray-600 text-sm italic border-r-4 border-gray-200 pr-3">"{{ $rev->comment }}"</p>
                                        @endif
                                    </div>
                                @endforeach
                            </div>
