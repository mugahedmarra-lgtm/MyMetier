            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-8 text-center">
                ...
                <div class="bg-gray-50 rounded-xl border border-gray-100 p-6 text-right max-w-md mx-auto space-y-3 mb-6">
                    <div class="flex justify-between text-sm">
                        ...
                        <span class="font-bold text-gray-900">{{ $provider_type === 'contractor' ? 'مقاول' : 'مهني' }}</span>
                    </div>
                    <div class="flex justify-between text-sm">
                    ...
                    <div class="flex justify-between text-sm">
                        ...
                        <span class="font-bold text-gray-900">{{ $display_name }}</span>
                    </div>
                </div>

                <a href="{{ route('dashboard') }}" class="inline-block bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-3 px-8 rounded-xl transition shadow-sm">
                    العودة للوحة التحكم
                </a>
            </div>

        {{-- ═══════════════════════════════════════════════════ --}}
        {{-- UPGRADE / RESUBMISSION FORM --}}
        {{-- ═══════════════════════════════════════════════════ --}}
        @else
            {{-- Rejection Banner --}}
            @if($isResubmission)
            ...
            <div class="bg-indigo-600 rounded-2xl shadow-md border border-indigo-700 p-8 mb-8 text-white relative overflow-hidden">
                ...
                <div class="relative z-10">
                    <h1 class="text-3xl font-extrabold tracking-tight mb-2">{{ $isResubmission ? 'تعديل وإعادة إرسال الطلب' : 'ترقية حسابك' }}</h1>
            ...
            <form wire:submit="submit" class="space-y-8">
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 md:p-8">
                    ...
                    @error('provider_type') <span class="text-red-500 text-xs mt-2 block">{{ $message }}</span> @enderror
                </div>
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 md:p-8">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            @error('secondary_phone') <span class="text-red-500 text-xs mt-1">{{ $message }}</span> @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <select wire:model.live="city_id" class="w-full text-right bg-white border border-gray-200 text-gray-900 rounded-xl px-4 py-3 focus:ring-2 focus:ring-indigo-500 focus:bg-white transition @error('city_id') border-red-500 ring-red-500 @enderror">
                        ...
                        <div>
                            ...
                            <select wire:model.live="district_id" class="w-full text-right bg-white border border-gray-200 text-gray-900 rounded-xl px-4 py-3 focus:ring-2 focus:ring-indigo-500 focus:bg-white transition @error('district_id') border-red-500 ring-red-500 @enderror" {{ empty($districts) ? 'disabled' : '' }}>
                ...
                {{-- ── Section 4: Contractor-Only Fields ─────────────── --}}
                @if($provider_type === 'contractor')
                    <div class="bg-white rounded-2xl shadow-sm border border-blue-100 p-6 md:p-8 ring-1 ring-blue-50">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                @error('contractor_type') <span class="text-red-500 text-xs mt-1">{{ $message }}</span> @enderror
                            </div>
                            ...
                            <div class="md:col-span-2">
                                ...
                                @error('team_size') <span class="text-red-500 text-xs mt-1">{{ $message }}</span> @enderror
                            </div>
                        </div>
                    </div>
                @endif
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 md:p-8">
                    <h2 class="text-lg font-bold text-gray-900 mb-5 flex items-center gap-2">
                        <span class="w-7 h-7 bg-indigo-100 text-indigo-600 rounded-full flex items-center justify-center text-sm font-bold">{{ $provider_type === 'contractor' ? '5' : '4' }}</span>
                        التحقق من الهوية
                    </h2>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            @error('identity_number') <span class="text-red-500 text-xs mt-1">{{ $message }}</span> @enderror
                        </div>
                        ...
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2">
                                ...
                                @if($isResubmission && $existingIdentityPath)
                                ...
                                <label for="identity-upload" class="flex flex-col items-center justify-center w-full h-32 border-2 border-gray-300 border-dashed rounded-xl cursor-pointer bg-gray-50 hover:bg-indigo-50 hover:border-indigo-300 transition relative overflow-hidden">
                                    ...
                                    @if($identity_image)
                                        <div class="absolute inset-0 z-10 bg-indigo-50/90 flex flex-col items-center justify-center">
                                            ...
                                            <p class="text-xs text-indigo-500 font-mono" dir="ltr">{{ $identity_image->getClientOriginalName() }}</p>
                                        </div>
                                    @elseif($isResubmission && $existingIdentityPath)
                            ...
                            @error('identity_image') <span class="text-red-500 text-xs mt-1">{{ $message }}</span> @enderror
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 md:p-8">
                    ...
                    <button type="submit"
                        ...
                        <span wire:loading.remove wire:target="submit">{{ $isResubmission ? 'إعادة إرسال الطلب' : 'إرسال طلب الترقية' }}</span>
